...
We assume that we have some covariants equilibria such that:
\begin{equation}
	\langle \delta \vec C_i^\star \vert \delta \vec C_j^\star \rangle > 0 
\end{equation}

\begin{equation}
\begin{array}{rl}
	 & \Xi_i\left(\vec C_0 + \beta_i \delta \vec C_i^\star + \sum_{j\not=i}  \beta_j\delta\vec C_j^\star \right) \\
	= & \Xi_i\left( \vec C_0 + (\beta_i + \sum_{j\not=i} \alpha_j) \delta \vec C_i^\star 
	+ \sum_{j\not=i}  \beta_j \delta\vec C_j^\star - \alpha_j \delta\vec C_i^\star \right)
	\\
\end{array}
\end{equation}

\begin{equation}
	\alpha_j = \beta_j \dfrac{\langle \delta \vec C_i^\star \vert \delta \vec C_j^\star \rangle}{ {\delta \vec C_i^\star}^2 }
\end{equation}

\begin{equation}
	 	\beta_j \delta\vec C_j^\star - \alpha_j \delta\vec C_i^\star = 
		\beta_j
		 \underbrace{\left(
		 \delta\vec C_j^\star 
		-  \dfrac{\langle \delta \vec C_i^\star \vert \delta \vec C_j^\star \rangle}{ {\delta \vec C_i^\star}^2 } \delta\vec C_i^\star
 	\right)}_{\delta\vec C_j^\perp}
\end{equation}

\begin{equation}
\begin{array}{rl}
 & \Xi_i\left( \vec C_0 + \delta\vec C_i^\star + (\beta_i -1 + \sum_{j\not=i} \alpha_j) \delta \vec C_i^\star 
	+ \sum_{j\not=i}  \beta_j \delta\vec C_j^\perp\right)\\
\simeq & (\beta_i -1 + \sum_{j\not=i} \alpha_j) \langle \vec \Phi_i^\star \vert \delta \vec C_i^\star \rangle
	+ \sum_{j\not=i}  \beta_j \langle \vec \Phi_i^\star \vert \delta\vec C_j^\perp \rangle\\
\end{array}
\end{equation}
