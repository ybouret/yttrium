...
\subsubsection{Encompassing Valid States}
We know that:
\begin{equation}
	\Xi_i(\vec C_0 + \xi_i^\star \vec \nu _i) = \Xi_i(\vec C_0 + \delta \vec C_i^\star) = \Xi_i(\vec C_i^\star) = 0
\end{equation}
Each $\vec C_i^\star$ is a valid concentration, so is:
\begin{equation}
	\vec C_\beta = \sum_j \beta_j \vec C_i^\star, \; \sum_j \beta_j = 1,\; \beta_j \in [0:1]
\end{equation}


With this assumptions:
\begin{equation}
	\vec C_\beta = \sum_j \beta_j (\vec C_0 + \delta \vec C_i^\star) = \vec C_0 + \sum_j \beta_j \delta \vec C_i^\star
\end{equation}

\subsubsection{Two Equilibria}

\begin{equation}
\left\lbrace
\begin{array}{rcll}
\Xi_1(\beta_1 \vec C_1^\star + \beta_2 \vec C_2^\star) & = & \Xi_1(\vec C_0 + \beta_1 \delta \vec C_1^\star + (1-\beta_1) \delta \vec C_2^\star), & \Xi_1(1)=0 \\
\Xi_2(\beta_1 \vec C_1^\star + \beta_2 \vec C_2^\star) & = & \Xi_2(\vec C_0 + \beta_1 \delta \vec C_1^\star + (1-\beta_1) \delta \vec C_2^\star), & \Xi_2(0)=0 \\
\end{array}
\right.
\end{equation}
