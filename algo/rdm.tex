...
\begin{equation}
	\Xi_i(\vec C_0 + \xi_i^\star \vec \nu_i) = 0
\end{equation}
For a perturbation $\delta \vec C$:
\begin{equation}
	\Xi_i(\vec C_0 + \delta \vec C + (\xi_i^\star + \delta \xi_i) \vec \nu_i) = 0 \iff 
	0 \simeq  \langle \vec \Phi_i^\star \vert \delta \vec C \rangle + \langle \vec \Phi_i^\star \vert \vec \nu_i \rangle \delta \xi_i 
\end{equation}
	
The new solving point is
\begin{equation}
	\xi_i^\circ = \xi_i^\star - \dfrac{\langle \vec \Phi_i^\star \vert \delta \vec C \rangle}{\langle \vec \Phi_i^\star \vert \vec \nu_i \rangle}
\end{equation}	

For $\delta \vec C = \bm{\nu}^T \vec \xi$:
\begin{equation}
	\vec \xi_\circ = \vec \xi_\star - \bm{\Upsilon} \vec \xi
\end{equation}	

We want to decrease:
\begin{equation}
	\mathfrak X(\vec \xi) = \dfrac{1}{2} {\vec \xi_\circ}^2 
	 = \dfrac{1}{2} \left[ {\vec \xi_\star}^2 
	 	- 2 \langle \vec \xi_\star \vert \bm{\Upsilon} \vert \vec \xi \rangle
		+ \langle \vec \xi \vert \bm{\Upsilon}^T \vert \bm{\Upsilon} \vert \vec \xi \rangle
	 \right]
\end{equation}

The descent direction is:
\begin{equation}
	\vec{\xi} \propto \bm{\Upsilon}^T  \vec \xi_\star
\end{equation}
