...
Let us assume that we start from $\vec C_0$, and we want to add the correction $\bm{P}\vec u$ such that:
\begin{equation}
	\bm{B}\left(\vec C_0 + \bm{P}\vec u \right) = \vec b
\end{equation}

\begin{equation}
	 \vec u = \vec b-\bm{B} \vec C _0 
\end{equation}

\begin{equation}
	\bm{P} \vec u = \underbrace{\bm{P} \vec b}_{\vec C^\star} - \bm{P} \bm{B} \vec C _0 
\end{equation}

\begin{equation}
	\vec C_0' = \vec C_0 + \bm{P}\vec u = \underbrace{\bm{P} \vec b}_{\vec C^\star} + \bm{Q} \vec C_0
\end{equation}
