...
namespace Yttrium
{
    ...
    namespace Crypto
    {
        void ARC4::init( const uint8_t *key, size_t len ) noexcept
        {
            x=y=0;
            S[0x00] = 0x00;
            S[0x01] = 0x01;
...
#endif
        ARC4:: ARC4( const Key &key ) throw() :
        x(0),
        y(0),
        S( )
        {
            init( (const uint8_t *)key.ro_addr(), key.measure() );
        }
        
        
        void ARC4:: schedule( const Key &key ) throw()
        {
            init( (const uint8_t *)key.ro_addr(), key.measure() );
        }
        
        const char * const ARC4::CallSign = "ARC4";
        
        const char * ARC4:: callSign() const noexcept
        {
            return CallSign;
        }
