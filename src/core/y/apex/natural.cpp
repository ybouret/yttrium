...
#define CONST_PROTO(ADDR) (*static_cast<const Prototype *>(ADDR))
        ...
        {
            return Natural(Prototype::Add( CONST_PROTO(lhs.impl), CONST_PROTO(rhs.impl), Number::addTicks),AsImpl);
        }

        Natural operator+(const Natural &lhs, const uint64_t rhs)
        {
            return Natural(Prototype::Add(CONST_PROTO(lhs.impl),rhs,Number::addTicks),AsImpl);
        }

        Natural operator+(const uint64_t lhs, const Natural &rhs)
        {
            return Natural(Prototype::Add(CONST_PROTO(rhs.impl),lhs,Number::addTicks),AsImpl);
        }

        Natural & Natural:: operator+=(const uint64_t rhs)
        {
            Natural res( Prototype::Add(CONST_PROTO(impl),rhs,Number::addTicks),AsImpl);
            xch(res);
            return *this;
        }
        ...
        {
            Natural res( Prototype::Add(CONST_PROTO(impl),CONST_PROTO(rhs.impl),Number::addTicks), AsImpl);
            xch(res);
            return *this;
        }
        ...
        {
            Natural res( Prototype::Add1(CONST_PROTO(impl),Number::addTicks), AsImpl);
            xch(res);
            return *this;
        }
