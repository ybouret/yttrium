...
namespace Yttrium
{
    namespace Apex
    {
        ...
        Block * Natural:: FFT_Mul(Block &          lhs,
                                  Block &          rhs)
        {
            ...
            if (carry >= 256.0) throw Specific::Exception("Apex::FFT_Mul","precision underflow");
...
namespace Yttrium
{
    namespace Apex
    {
        Natural Natural:: FFT(const Natural &lhs, const Natural &rhs)
        {
            Y_Apex_Lock(lhs);
            Y_Apex_Lock(rhs);
            return Natural(FFT_Mul(*lhs.block,*rhs.block),AsBlock);
        }
    }
}
...
namespace Yttrium
{
    namespace Apex
    {
        ...
        Block * Natural:: FFT_Sqr(Block &          lhs)
        {
            static Factory & factory = Factory::Instance();
            const Jig1 & u = lhs.make<Plan1>(); const size_t n = u.words; if(n<=0) return factory.query(0);
            ...
            BlockPtr     blk( factory.queryBytes( (nn) * sizeof(Real)  ) ); assert(blk->range >= (nn) * sizeof(Real) );
            Real * const b = static_cast<Real *>(blk->entry) - 1;
            {
                //Real * const a = b+nn;
                ...
                //
                //--------------------------------------------------------------
                for(size_t i=n;i>0;--i) b[i] = u.word[n-i];
                //for(size_t i=m;i>0;--i) b[i] = v.word[m-i];
            ...
            if (carry >= 256.0) throw Specific::Exception("Apex::FFT_Sqr","precision underflow");
