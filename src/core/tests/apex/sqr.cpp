...
Y_UTEST(apex_sqr)
{
    ...
    for(size_t lbits=minBits;lbits<=maxBits;lbits <<= 1)
    {
        ...
        (std::cerr << std::setw(5) << lbits).flush();
        ...
        do
        {
            ...
            const Natural  sqr( Natural::BFA_Sqr( Coerce(*lhs) ), AsBlock );
            mulTMX += WallTime::Ticks() - mmark;

            // timing FFT sqr
            lhs.plan(ran);
            const uint64_t fmark = WallTime::Ticks();
            const Natural  fft( Natural::FFT_Sqr( Coerce(*lhs) ), AsBlock );
