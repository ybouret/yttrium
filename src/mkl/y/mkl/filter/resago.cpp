...
namespace Yttrium
{
    namespace MKL
    {
        ...
        public:
            ...
            f(filter.f.rows,filter.f.cols)
            {
                for(size_t i=1;i<=f.rows;++i)
                {
                    ...
                    {
                        Coerce(f[i][j]) = filter.f[i][j].toReal<T>();
                    }
                }
            }
        ...
        template <typename T>
        class SavGol<T>::Code : public Object, public SavGolFilter<T>::DataBase
        {
        public:
            //__________________________________________________________________
            //
            // Definitions
            //__________________________________________________________________
            typedef SavGolFilter<T>                    FilterType;     //!< alias
            typedef typename SavGolFilter<T>::Pointer  FilterPointer;  //!< alias
            typedef typename SavGolFilter<T>::DataBase FilterDB;       //!< alias
            ...
            // Methods
            //__________________________________________________________________
            inline const FilterType &get(const int32_t  n,
                                         const int32_t  j,
                                         const uint32_t d)
            {
                ...
                const ReSaGoFilter &impl = raw.get(key);
