...
Y_UTEST(fit_samples)
{
...

    {
        ...
        Y_ASSERT(Fit::Failure != xfit.run(F,S1,aorg,dom,used,G,aerr,xml) );
        S1.saveDatFile("-try");
        ...
        Y_ASSERT(Fit::Failure != xfit.run(F,S2,aorg,dom,used,G,aerr,xml) );
        S2.saveDatFile("-opt");
        S2.vars.display("", std::cerr, aorg);


        Y_ASSERT( Fit::Failure != xfit.run(F,samples,aorg,dom,used,G,aerr,xml) );
